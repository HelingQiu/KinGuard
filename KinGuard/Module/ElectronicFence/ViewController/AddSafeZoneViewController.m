//
//  AddSafeZoneViewController.m
//  KinGuard
//
//  Created by Rainer on 16/7/12.
...
@interface AddSafeZoneViewController ()<MAMapViewDelegate>
...
@implementation AddSafeZoneViewController
...
//    [self initilizeBaseView];
    ...
    [[KinLocationApi sharedKinLocation] startNormalLocation:pid success:^(NSDictionary *data) {
        [[KinLocationApi sharedKinLocation] readLocationInfo:pid success:^(NSDictionary *data) {
            [JJSUtil hideHUD];
            NSLog(@"safe:%@",data);
            self.safeModel = [SafeZoneModel mj_objectWithKeyValues:data];
            [self initilizeBaseView];
        } fail:^(NSString *error) {
            [JJSUtil hideHUD];
            NSLog(@"dd%@",error);
...
- (void)initilizeBaseView
{
    ...
    [labRadius setText:[[@"范围：" stringByAppendingString:@"500"] stringByAppendingString:@"米"]];
